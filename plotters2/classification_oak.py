...
markers = ['s', 'P', 'D', '^', 'o', '*', '.']
labels = ["Overall Accuracy", r"Cohen's kappa ($\kappa$)"]
...
for metric_index,metric in enumerate(["metric1", "metric2"]):
    for ds_index, dataset in enumerate(datasets):
        ...
        #     legend = axes[metric_index, ds_index].legend(ncols=3, loc='upper left', fontsize=18,bbox_to_anchor=(1.05, 1))
        #     legend.get_title().set_fontsize('18')
        #     # legend.get_title().set_fontweight('bold')
        #     for text in legend.get_texts():
        #         if text.get_text() == 'Proposed algorithm':
        #             text.set_fontweight('bold')
        axes[metric_index, ds_index].set_xticks([5,10,15,20,25])
        axes[metric_index, ds_index].set_ylim(0.48, 0.78)
        axes[metric_index, ds_index].set_xticklabels(["5","10","15","20","25"])
