...
df = pd.read_csv(loc)
df = df[["epoch","validation_rmse"]]
limit = 8000
df = df[df["epoch"]<limit]
fig = go.Figure()


additional_trace = go.Scatter(x=df["epoch"], y=df[f"validation_rmse"], mode='lines', name=f'$R^2')
...
if not os.path.exists(subfolder):
    os.mkdir(subfolder)
path = os.path.join(subfolder, f"rmse_{idx}.png")
