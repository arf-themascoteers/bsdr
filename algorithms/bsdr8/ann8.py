...
class ANN8(nn.Module):
    def __init__(self, target_size, class_size,structure=None):
        ...
        self.indices = nn.Parameter(torch.tensor([ANN8.inverse_sigmoid_torch(init_vals[i + 1]) for i in range(self.target_size)], requires_grad=True).to(self.device))
        if structure == None:
            structure = [128,64]
    ...
    def forward(self, linterp):
        outputs = torch.stack([linterp.evaluate(index) for index in self.get_indices()], dim=1)
